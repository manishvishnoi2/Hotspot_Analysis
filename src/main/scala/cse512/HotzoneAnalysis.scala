...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    var hotZones = spark.sql("select rectangle,count(point) as pointsContained from joinResult group by rectangle order by rectangle")
    hotZones.createOrReplaceTempView("hotzoneResult")
    val count = spark.sql("select count(*) from hotzoneResult")

    // for getting one output part file
    hotZones = spark.sql("select * from hotzoneResult limit " + count.first().getLong(0).toString())
    hotZones.show()
    return hotZones 
  }
